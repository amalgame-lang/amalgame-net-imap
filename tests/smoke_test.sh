...
#  Builds a real ImapServer on 127.0.0.1:2527 (self-signed cert, one
#  AUTH account), drives it with a Python smtplib client doing
#  STARTTLS → LOGIN → SELECT → FETCH, and asserts the message was delivered
...
BUILD_DIR="$(mktemp -d -t ani-smoke-XXXXXX)"
...
for i in $(seq 1 50); do
    grep -q "SMOKE-SERVER-READY" "$BUILD_DIR/srv.out" 2>/dev/null && \
      python3 -c "import socket; socket.create_connection(('127.0.0.1',2527),1).close()" 2>/dev/null && break
    sleep 0.2
done

echo "── client: STARTTLS → LOGIN → SELECT → FETCH ──"
python3 - <<'PYC'
import imaplib, ssl, sys
ctx = ssl.create_default_context(); ctx.check_hostname=False; ctx.verify_mode=ssl.CERT_NONE
try:
    M = imaplib.IMAP4("127.0.0.1", 2527)
    M.starttls(ssl_context=ctx)
    M.login("alice", "s3cret")
    M.select("INBOX")
    typ, data = M.search(None, "ALL")
    ids = data[0].split()
    assert ids, "no messages"
    typ, d = M.fetch(ids[0], "(BODY[])")
    body = b""
    for part in d:
        if isinstance(part, tuple) and len(part) > 1 and part[1]:
            body += part[1]
    assert b"hello imap smoke" in body, ("body not found: %r" % body[:120])
    M.logout()
    print("CLIENT-OK")
...
echo "── assert client read ──"
PASS=0; FAIL=0
[ "$CLIENT_RC" -eq 0 ] && PASS=$((PASS+1)) || { FAIL=$((FAIL+1)); echo -e "  ${RED}client failed${NC}"; }
if [ "$CLIENT_RC" -ne 0 ]; then echo "  server log:"; sed 's/^/    /' "$BUILD_DIR/srv.out"; fi
echo "────────────────────────────────────────────"
